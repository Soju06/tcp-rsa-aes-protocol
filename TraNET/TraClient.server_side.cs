        ...
        internal virtual async Task Process_server_side(TraServer server, TRACLIENTINFO traInfo, CancellationToken token) {
...

            {
                ...
                if (!TraVersion.CanSupport(TraNet.ProtocolVersion, clientInfo.ProtocolVersion)) {
                    await SendMessageAsync(TraStatusCode.NotSupport, 3);
                ...
                if (!TraBuffer.EqualBuffer(vef_buf, clientInfo.ProtocolName.ToArray())) {
                    await SendMessageAsync(TraStatusCode.NotSupport, 1);
                    throw new ProtocolMismatchException("프로토콜 이름이 일치하지 않습니다.");
                }

                // 만약 차단되면
                if (server.RaiseConnectionEvent(endPoint, 
                    clientInfo.ProtocolVersion, clientInfo.ProtocolName, out var blockCode)) {
                    await SendMessageAsync(TraStatusCode.Block, blockCode);
                }
...

            {
                ...
                if (!TraBuffer.EqualBuffer(vef_hash = TraCrypto.GetSHA256Hash(new[] { vef_swap }), aes.Decrypt(data, tag))) {
                    await SendMessageAsync(TraStatusCode.Cancel, 13);
                    throw new ProtocolViolationException("대상이 암호화 인증을 실패하였습니다.");
                }
...

            {
                ...
                server.RaiseSessionStartEvent(endPoint, ref bufferSize, out var state);
                ...
                if (!TraBuffer.EqualBuffer(vef_hash, aes.Decrypt(data, tag))) {
                    await SendMessageAsync(TraStatusCode.Cancel, 17);
            ...
            async Task SendMessageAsync(TraStatusCode statusCode, int code) =>
